...
<div class="p-6 lg:p-8 max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-800">Add Event</h1>
        <p class="text-gray-600 mt-1">Set reminders for important occasions</p>
    </div>
    
    <form action="{{ route('events.store') }}" method="POST" class="bg-white rounded-2xl shadow-lg p-6 space-y-6">
        @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="event_type" 
                            ...
                            <option value="custom" {{ old('event_type') == 'custom' ? 'selected' : '' }}>📅 Custom</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="remind_days_before" 
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary">
                        <option value="1" {{ old('remind_days_before') == '1' ? 'selected' : '' }}>1 day before</option>
                        <option value="3" {{ old('remind_days_before', '3') == '3' ? 'selected' : '' }}>3 days before</option>
                        <option value="7" {{ old('remind_days_before') == '7' ? 'selected' : '' }}>1 week before</option>
                        <option value="14" {{ old('remind_days_before') == '14' ? 'selected' : '' }}>2 weeks before</option>
                        <option value="30" {{ old('remind_days_before') == '30' ? 'selected' : '' }}>1 month before</option>
                    </select>
                </div>
