...
<div class="p-6 lg:p-8 max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-800">Add Contact</h1>
        <p class="text-gray-600 mt-1">Save contact details to send cards easily</p>
    </div>
    
    <form action="{{ route('contacts.store') }}" method="POST" class="bg-white rounded-2xl shadow-lg p-6 space-y-6">
        @csrf
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="relationship" 
                        ...
                        <option value="Partner" {{ old('relationship') == 'Partner' ? 'selected' : '' }}>Partner</option>
        ...
        <div>
            ...
            <textarea name="notes" 
                      ...
                      class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary resize-none @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
            @error('notes')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
